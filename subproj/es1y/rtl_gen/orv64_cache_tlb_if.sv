...
`ifdef PYGMY_ES1Y
`ifndef ORV64_CACHE_TLB_IF__SV
`define ORV64_CACHE_TLB_IF__SV
`define orv64_cache_tlb_if_slave_assign(intf, st) \
  ...
  ``intf``.resp_ppn = ``st``.resp.resp_ppn; \
  ``st``.resp_ready = ``intf``.resp_ready; \

`define orv64_cache_tlb_if_slave_wire(intf) \
  logic ``intf``_req_valid; \
  logic ``intf``_req_ready; \
  logic ``intf``_resp_valid; \
  logic ``intf``_resp_ready; \
  orv64_cache_tlb_if_req_t ``intf``_req; \
  orv64_cache_tlb_if_resp_t ``intf``_resp; \

`define orv64_cache_tlb_if_slave_wire_arr(intf, N) \
  logic ``intf``_req_valid[``N``]; \
  logic ``intf``_req_ready[``N``]; \
  logic ``intf``_resp_valid[``N``]; \
  logic ``intf``_resp_ready[``N``]; \
  orv64_cache_tlb_if_req_t ``intf``_req[``N``]; \
  orv64_cache_tlb_if_resp_t ``intf``_resp[``N``]; \

`define orv64_cache_tlb_if_slave_port(intf) \
  input logic ``intf``_req_valid, \
  output logic ``intf``_req_ready, \
  output logic ``intf``_resp_valid, \
  input logic ``intf``_resp_ready, \
  input orv64_cache_tlb_if_req_t ``intf``_req, \
  output orv64_cache_tlb_if_resp_t ``intf``_resp

`define orv64_cache_tlb_if_slave_port_arr(intf, N) \
  input logic ``intf``_req_valid[``N``], \
  output logic ``intf``_req_ready[``N``], \
  output logic ``intf``_resp_valid[``N``], \
  input logic ``intf``_resp_ready[``N``], \
  input orv64_cache_tlb_if_req_t ``intf``_req[``N``], \
  output orv64_cache_tlb_if_resp_t ``intf``_resp[``N``]

`define orv64_cache_tlb_if_slave_port2struct_assign(intf, st) \
...
`define orv64_cache_tlb_if_slave_port2struct_arr_assign(intf, st, N) \
...
`define orv64_cache_tlb_if_master_assign(intf, st) \
  ...
  ``st``.resp.resp_ppn = ``intf``.resp_ppn; \
  ``intf``.resp_ready = ``st``.resp_ready; \

`define orv64_cache_tlb_if_master_wire(intf) \
  logic ``intf``_req_valid; \
  logic ``intf``_req_ready; \
  logic ``intf``_resp_valid; \
  logic ``intf``_resp_ready; \
...
`define orv64_cache_tlb_if_master_wire_arr(intf, N) \
  logic ``intf``_req_valid[``N``]; \
  logic ``intf``_req_ready[``N``]; \
  logic ``intf``_resp_valid[``N``]; \
  logic ``intf``_resp_ready[``N``]; \
...
`define orv64_cache_tlb_if_master_port(intf) \
  output logic ``intf``_req_valid, \
  input logic ``intf``_req_ready, \
  input logic ``intf``_resp_valid, \
  output logic ``intf``_resp_ready, \
  output orv64_cache_tlb_if_req_t ``intf``_req, \
  input orv64_cache_tlb_if_resp_t ``intf``_resp

`define orv64_cache_tlb_if_master_port_arr(intf, N) \
  output logic ``intf``_req_valid[``N``], \
  input logic ``intf``_req_ready[``N``], \
  input logic ``intf``_resp_valid[``N``], \
  output logic ``intf``_resp_ready[``N``], \
  output orv64_cache_tlb_if_req_t ``intf``_req[``N``], \
  input orv64_cache_tlb_if_resp_t ``intf``_resp[``N``]

`define orv64_cache_tlb_if_master_port2struct_assign(intf, st) \
...
`define orv64_cache_tlb_if_master_port2struct_arr_assign(intf, st, N) \
...
interface orv64_cache_tlb_if (input clk, rstn);
