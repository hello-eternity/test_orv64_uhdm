...
class tsi_t : public htif_t
{
 ...
 protected:
  void reset() override;
  void read_chunk(addr_t taddr, size_t nbytes, void* dst, enum rw_target target = L2) override;
  void write_chunk(addr_t taddr, size_t nbytes, const void* src, enum rw_target target = L2) override;
