...
enum rw_target {DMA = 0, L2 = 1, DDR = 2, DMA_BURST = 3};
...
public:
  ...
  virtual void read(addr_t addr, size_t len, void* bytes, enum rw_target target = DMA);
  virtual void write(addr_t addr, size_t len, const void* bytes, enum rw_target target = DMA);


  //virtual void read_debug(addr_t addr, size_t len, void* bytes, enum rw_target target = L2);
  //virtual void write_debug(addr_t addr, size_t len, const void* bytes, enum rw_target target = L2);
