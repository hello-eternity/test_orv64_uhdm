    ...
    for (unsigned i = 0; i < eh->e_phnum; i++) { \
      if(ph[i].p_type == PT_LOAD && ph[i].p_memsz) { \
        if (ph[i].p_filesz) { \
          ...
          /*memif->write(ph[i].p_paddr, ph[i].p_filesz, (uint8_t*)buf + ph[i].p_offset, DDR);*/\
          memif->write(ph[i].p_vaddr, ph[i].p_filesz, (uint8_t*)buf + ph[i].p_offset);\
        } \
        zeros.resize(ph[i].p_memsz - ph[i].p_filesz); \
        /*memif->write(ph[i].p_paddr + ph[i].p_filesz, ph[i].p_memsz - ph[i].p_filesz, &zeros[0], DDR);*/\
        memif->write(ph[i].p_vaddr + ph[i].p_filesz, ph[i].p_memsz - ph[i].p_filesz, &zeros[0]);\
      } \
    } \
...
{
  fprintf(stderr, "load_elf_symbols_only, fn = %s\n", fn);
  int fd = open(fn, O_RDONLY);
  struct stat s;
  assert(fd != -1);
  if (fstat(fd, &s) < 0)
  ...
  #define LOAD_ELF_SYMBOLS_ONLY(ehdr_t, phdr_t, shdr_t, sym_t) do { \
    ehdr_t* eh = (ehdr_t*)buf; \
    phdr_t* ph = (phdr_t*)(buf + eh->e_phoff); \
    *entry = eh->e_entry; \
    assert(size >= eh->e_phoff + eh->e_phnum*sizeof(*ph)); \
  ...
  if (IS_ELF32(*eh64))
    LOAD_ELF_SYMBOLS_ONLY(Elf32_Ehdr, Elf32_Phdr, Elf32_Shdr, Elf32_Sym);
  else
    LOAD_ELF_SYMBOLS_ONLY(Elf64_Ehdr, Elf64_Phdr, Elf64_Shdr, Elf64_Sym);
