...
class ContextClass(object):
    ...
    def to_bin_str(self): # changed by J.Wang
        return "%s %s %s" % (str(self.signbit), str(self.characteristic),
                             str(self.significand))

    def to_hex_str(self): # changed by J.Wang
        return hex(int(self.to_bin_str().replace(' ', ''), 2))
...
if __name__ == '__main__':
    print 'input IEEE-754 floating number in decimal or hex format, and we will convert it into the other format. Default is single precision (32-bit) mode, use "double" or "single" to change mode'
    is_single = True
    while (True):
        if (is_single):
            tt_len = 32
        ...
        if (raw == 'single'):
            print('change to single precision')
            is_single = True
        elif (raw == 'double'):
            print('change to double precision')
            is_single = False
        elif (raw.startswith('0x')):
            # hex
            x = context(raw)
            print '%s    %s' % (x.as_decimal(), x.to_hex_str())
        else:
            # decimal
            x = context(Decimal(raw))
            print '%s    %s' % (x.as_decimal(), x.to_hex_str())
