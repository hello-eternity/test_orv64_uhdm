...
`ifndef __OT_RR_ARBITER_RSTN_V__
`define __OT_RR_ARBITER_RSTN_V__
module ot_rr_arbiter_rstn 
    ...
    assign grt = (ready) ? grt_comp(.rr_state(rr_state), .req(req)) : {input_cnt{1'b0}};
    
    // compute nxt_rr_state based on grt
    assign nxt_rr_state = (grt != {input_cnt{1'b0}}) ? grt : rr_state;
