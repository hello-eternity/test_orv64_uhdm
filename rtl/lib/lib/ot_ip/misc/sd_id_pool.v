...
//===========================================================================================
// Description:
//    A pool of IDs which are used in CAM control or out-of-order buffers
...
`ifndef __SD_ID_POOL_V__ 
`define __SD_ID_POOL_V__ 
...
module sd_id_pool 
    ...
    output [ptr_sz:0]      usage     // how many entries in the pool have been used
    ...
    generate
        for(ii=0; ii<depth; ii=ii+1) begin: array_entry_ii
            always @(posedge clk) begin
                if (rst) begin
                    array[ii] <= ii[width-1:0]; // initalized with a full pool of ids
                end
    ...
    assign p_data = array[rd_addr]; // ri lint_check_waive
    
    //=============== update wr_addr and rd_addr
    assign nxt_wr_addr = (wr_en) ? ((wr_addr==depth_sub_1[ptr_sz-1:0]) ? {ptr_sz{1'b0}} : wr_addr+1'b1):
                         wr_addr;
    ...
    assign nxt_rd_addr = (rd_en) ? ((rd_addr==depth_sub_1[ptr_sz-1:0]) ? {ptr_sz{1'b0}} : rd_addr+1'b1):
                         rd_addr;
...
//spyglass enable_block W71           
    ...
    always @(posedge clk) begin
        if (rst) begin
            occupy_cnt <= depth[ptr_sz:0]; // initialized to be full
        end
        else begin
